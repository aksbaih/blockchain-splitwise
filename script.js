...
async function getTotalOwed(user) {
  return getUsers().then(async (users) => {
    let totalOwed = 0;

    for (let creditor of users) {
      if (creditor === user) continue;

      totalOwed += await BlockchainSplitwise.methods
        .lookup(user, creditor)
        .call();
    }
...
async function getLastActive(user) {
  return getAllFunctionCalls(contractAddress, "add_iou").then(
    (functionCalls) => {
      // might need to reverse array
      const found = functionCalls.find((call) => {
        call.from === user || call.args.sender === user;
      });
...
async function getAllFunctionCalls(addressOfContract, functionName) {
  ...
  // Does this timestamp work?
  return function_calls.sort((a, b) => (a.t > b.t ? 1 : -1));
}
